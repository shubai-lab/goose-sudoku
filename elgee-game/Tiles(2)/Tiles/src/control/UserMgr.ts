...
declare function getUrlParam(name): string;
...
export default class UserMgr {
    ...
    public data = { expires_in: 0, token: "" };

    private _musicSet = true;

    public get musicSet() {
        return this._musicSet;
    }
    // 是否跳过Http请求
    private jumpHttp = true;
    //游戏gameId
    private game_id = 0;
    //微信授权code
    private code = "";
    //token最小有效时长 
    private MIN_TOKEN_TIME = 60 * 60;
    //游戏地址
    public GameUrl = "https://10thwxh5.sanguosha.com/slgs/index.html";
    //是否获取到了消息 
    public isGetInfo = false;
     ...
     * @param fail 失败回调
     */
    public UseProp(suc, fail) {
        if (this.jumpHttp) { suc && suc(); return; }
        if (this._info.share_item <= 0) return;
    ...
    public WxTestLogin(code, suc, fail) {
